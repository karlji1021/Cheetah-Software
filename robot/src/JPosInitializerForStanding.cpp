...
void JPosInitializer<T>::_UpdateInitial(const LegController<T>* ctrl) {
  ...
  mid[1] = new T[cheetah::num_act_joint];
...
//    mid[0][i] = _mid_jpos1[i];
//    mid[1][i] = _mid_jpos2[i];
  }
...
//  handler.getVector<T>("mid_jpos1", _mid_jpos1);
//  handler.getVector<T>("mid_jpos2", _mid_jpos2);
}
